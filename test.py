...
def test(imgs_path=TEST_IMG_DIR, ckpts_path=OUTPUT_ROOT_DIR):
    ...
    cv2.imshow('resize', np.array(images[-3]))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    images, labels = flip_image(images, labels)
    cv2.imshow('flip', np.array(images[-3]))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    images, labels = crop_image(images, labels)
    cv2.imshow('crop', np.array(images[-3]))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    images, labels = fancy_pca(images, labels)
    cv2.imshow('pca', np.array(images[-3]))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    test_X, test_Y = make_dataset(images, labels)
    cv2.imshow('dataset', np.array(test_X[-3]))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
